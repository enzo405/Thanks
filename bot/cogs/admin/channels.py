...
class Channel(commands.Cog):
	...
	@app_commands.command(name="channel_whitelist",description="Add the channel to the list of channel where the bot will interact")
	async def channel_whitelist(self,interaction:discord.Interaction, channel: Union[discord.TextChannel, discord.ForumChannel]):
		if not interaction.user.guild_permissions.administrator:
			await interaction.response.send_message("Only server administrator can use this command", ephemeral=True)
			return
		
		if channel.id not in [channel["channel_id"] for channel in self.bot.guilds_config[channel.guild.id]["blacklisted_channel"]]:
	...
	async def channel_blacklist(self,interaction:discord.Interaction, channel: Union[discord.TextChannel, discord.ForumChannel]):
		if not interaction.user.guild_permissions.administrator:
			await interaction.response.send_message("Only server administrator can use this command", ephemeral=True)
			return

		if channel.id in [channel["channel_id"] for channel in self.bot.guilds_config[channel.guild.id]["blacklisted_channel"]]:
